...
VThemesWidget::VThemesWidget(QWidget* parent) : QLWidget(parent)
{
	...
	m_theme_buttons_handler_connection =
		connect(vApp, &VApplication::active_theme_changed, [this]
			{
	...
	m_spacer_1->fill()->create_link(this->fill());

	m_spacer_2->setFixedWidth(12);
	m_spacer_2->fill()->create_link(this->fill());

	init_layout();
}

VThemesWidget::~VThemesWidget()
{
	QObject::disconnect(m_theme_buttons_handler_connection);
}

void VThemesWidget::handle_theme_buttons_visibility()
{
	if (activeTheme()->editable())
...
QLButton* VThemesWidget::customize_theme_button() const
{
	return m_customize_theme_button;
}

QLButton* VThemesWidget::new_theme_button() const
{
	return m_new_theme_button;
}

VThemeComboBox* VThemesWidget::theme_combobox() const
{
	return m_theme_combobox;
}

void VThemesWidget::show_custom_theme_buttons(bool cond)
{
	if (cond)
	{
...
void VThemesWidget::init_layout()
{
