...
#include <VortexWidgets/vslider.h>
...
using Vortex::VSlider;

VSlider::VSlider(QWidget* parent) :
	m_is_decimal_slider{ true }, QWidget(parent)
{
	init();
}

VSlider::VSlider(int limit, QWidget* parent) :
	m_is_decimal_slider{ false }, m_limit{ limit }, QWidget(parent)
{
	init();
}

void VSlider::set_limit(int limit)
{
	m_limit = limit;
...
LAttribute* VSlider::value()
{
	return m_value;
}

bool VSlider::eventFilter(QObject* object, QEvent* event)
{
...
void VSlider::update_handle_pos()
{
	if (m_is_decimal_slider)
	{
		float range = float(width() - m_handle->width());
