...
VTab* VMainWindow::open_central_widget(
	QWidget* central_widget, std::unique_ptr<VGraphic> tab_icon_graphic,
...
void VMainWindow::update()
{
	...
	{
		LStyle* style = Vortex::get_widget_lstyle(this);
...
bool VMainWindow::event(QEvent* e) 
{
	if (e->type() == QEvent::WindowStateChange ||
		e->type() == Vortex::VStyleAppliedEvent::EventType)
	{
		update();
	}
...
{
	connect(m_titlebar->settings_button(), &VButton::clicked,
		[this]
		{
	...
	connect(m_titlebar->minimize_button(), &VButton::clicked,
		[this]
		{
			showMinimized();
		});

	connect(m_titlebar->maximize_button(), &VButton::clicked,
		[this]
		{
			if (isMaximized())
				showNormal();
	...
	connect(m_titlebar->exit_button(), &VButton::clicked,
		[this] { qApp->quit(); });
}
...
void VMainWindow::_open_central_widget(QWidget* central_widget)
{
	...
	Vortex::apply_layers_style_from_parent(central_widget);
